...
const BRAND_COMMANDS = new Set(['plgames', 'plg']);
const BRAND_CLI_NAME = 'plgames';

export class RunCommand extends PackageCommand {
  static override paths = [[], ['run'], ['r']];

  static override usage = PackageCommand.Usage({
    description: `${BRAND.productName} Monorepo scripts`,
    details: `
      \`${BRAND_CLI_NAME} web <script>\`    Run any script defined in package's package.json

      \`${BRAND_CLI_NAME} init\`            Generate the required files if there are any package added or removed

      \`${BRAND_CLI_NAME} clean\`           Clean the output files of ts, cargo, webpack, etc.

      \`${BRAND_CLI_NAME} bundle\`          Bundle the packages

      \`${BRAND_CLI_NAME} build\`           A proxy for <-p package>'s \`build\` script

      \`${BRAND_CLI_NAME} dev\`             A proxy for <-p package>'s \`dev\` script
    `,
    examples: [
  ...
  async runScript(
    ...
    const shouldUseBrandCli = args[0] && BRAND_COMMANDS.has(args[0]);
    if (shouldUseBrandCli) {
      // remove command name from '<brand> xxx' command
      args.shift();
