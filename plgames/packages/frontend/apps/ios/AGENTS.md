...
1. `BUILD_TYPE=canary PUBLIC_PATH="/" yarn plgames @affine/ios build` - Build web assets
2. `yarn plgames @affine/ios cap sync` - Sync with iOS project
3. `yarn plgames @affine/ios cap open ios` - Open in Xcode

### Live Reload Setup

1. Run `yarn dev` and select `ios` for Distribution option
2. Run `yarn plgames @affine/ios sync:dev`
3. Run `yarn plgames @affine/ios cap open ios`
