...
const PaymentRequiredErrorRenderer = (host?: EditorHost | null) => html`
  <ai-error-wrapper
    .text=${`You've reached the current usage cap for ${BRAND.aiName}. You can subscribe to ${BRAND.aiName} (with a free 7-day trial) to continue the AI experience!`}
    .actionText=${'Upgrade'}
...
const LoginRequiredErrorRenderer = (host?: EditorHost | null) => html`
  <ai-error-wrapper
    .text=${`You need to log in to ${BRAND.cloudName} to continue using ${BRAND.aiName}.`}
    .actionText=${'Login'}
...
const GeneralErrorRenderer = (props: ErrorProps = {}) => {
  const onClick = () => {
    window.open(`mailto:${BRAND.contact.supportEmail}`, '_blank');
  };
  ...
  return html`<ai-error-wrapper
    ...
    .actionTooltip=${props.actionTooltip ?? BRAND.contact.supportEmail}
    .onClick=${onClick}
  ></ai-error-wrapper>`;
};
