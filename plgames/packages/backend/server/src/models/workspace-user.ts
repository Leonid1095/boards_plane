...
import type {
  WorkspaceMemberSource as WorkspaceMemberSourceType,
  WorkspaceMemberStatus as WorkspaceMemberStatusType,
  WorkspaceUserRole,
} from '@prisma/client';
import pkg from '@prisma/client';
const { WorkspaceMemberSource, WorkspaceMemberStatus } = pkg;
...
export class WorkspaceUserModel extends BaseModel {
  ...
  async set(
    ...
    role: WorkspaceRole,
    defaultData: {
      status?: WorkspaceMemberStatusType;
      source?: WorkspaceMemberSourceType;
      inviterId?: string;
    } = {}
  ) {
  ...
  async allocateSeats(workspaceId: string, limit: number) {
    ...
    ) as Record<WorkspaceMemberSourceType, WorkspaceUserRole[]>;
