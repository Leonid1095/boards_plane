...
import type { Update, Prisma as PrismaNamespace } from '@prisma/client';
...
export type DocMetaUpsertInput = Omit<
  PrismaNamespace.WorkspaceDocUncheckedCreateInput,
  'workspaceId' | 'docId'
>;
...
export class DocModel extends BaseModel {
  ...
  async getSnapshot<Select extends PrismaNamespace.SnapshotSelect>(
    workspaceId: string,
    docId: string,
    ...
    })) as PrismaNamespace.SnapshotGetPayload<{ select: Select }> | null;
  }
   ...
   * Get the doc meta.
   */
  async getMeta<Select extends PrismaNamespace.WorkspaceDocSelect>(
    workspaceId: string,
    docId: string,
    options?: {
      select?: Select;
    ...
    return (await this.db.workspaceDoc.findUnique({
      ...
      select: options?.select,
    })) as PrismaNamespace.WorkspaceDocGetPayload<{ select: Select }> | null;
  }
  ...
  async findMetas<Select extends PrismaNamespace.WorkspaceDocSelect>(
    ...
    const rows = (await this.db.workspaceDoc.findMany({
      ...
      select,
    })) as (PrismaNamespace.WorkspaceDocGetPayload<{ select: Select }> & {
      workspaceId: string;
      docId: string;
    })[];
    const resultMap = new Map(
